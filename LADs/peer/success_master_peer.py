...
layout = html.Div([
            dcc.Location(id='url_logout_peer', refresh=True),
            html.Br(),
            html.Div([
                html.Div([
                    html.Div([
                    ...
                    html.H6(id='user-div-peer', 
                            ...
                            style = {'width':'80%'}),
                html.Button('Logout', n_clicks=0, id = 'logout-button-peer', className="six columns",
                            style = {'width':'15%'})
                    ], 
                    className='row'),
            html.Div([
                html.Div([
                    ...
                    html.Div([
                        html.Div(id='ha1-output-peer',
                            className='six columns'),
                        html.Div(id='ha2-output-peer',
                            className='six columns')]
                            )
                        ],
                        className='six columns',
                ...
                html.Div([
                    html.Div([
                        ...
                        # html.Img(id='image-tooltip-practice-test', src = 'assets/tooltip.svg', style={'flex': '4%'},
                        #          className="six columns", title=tooltips_texts['PracticeTest']
                        #         )
                        html.Div([
                    ...
                    html.Div(id='practice-test-markdown-peer'),
                        ],
                        className='six columns',
                        style={"border": "1px solid grey"}
                        )
            ...
            html.Div([
                html.Div([
                    ...
                    # html.Img(id='image-tooltip-edx-es', src = 'assets/tooltip.svg', style={'flex': '2%'},
                    #          className="six columns", title=tooltips_texts['edXES']
                    #         )
                    html.Div([
                ...
                html.Div([
                    html.Div([
                        dcc.Graph(id='perf-avgquiz-plot-peer')
                            ],
                            # className='six columns'
                            ), 
                    html.Div([
                        dcc.Graph(id='perf-quizzes-plot-peer')
                            ],
                            # className='six columns'
                            )
                        ],
            ...
            html.Div([
                html.Div([
                    ...
                    # html.Img(id='image-tooltip-edx-quizzes', src = 'assets/tooltip.svg', style={'flex': '2%'},
                    #          className="six columns", title=tooltips_texts['edXQuizzes']
                    #         )
                    html.Div([
                ...
                html.Div([
                    html.Div([
                        html.Div([
                            ...
                            dcc.Graph(id='avgquiz-plot-peer')
                                ])
                            ],
                            # className='six columns'
                            ), 
                    html.Div([
                        html.Div([
                            ...
                            dcc.Graph(id='quizzes-plot-peer')
                                ]
                                )
                            ],
                            # className='six columns'
            ...
            html.Div([
                html.Div([
                    ...
                    dcc.Graph(id='models-line-plot-peer')
                        ],
                        className='six columns',
                        style={"border": "1px solid grey"}
                        ),
                html.Div([
                    ...
                    html.Div([
                        html.Div(id="legend-peer",
                                style={'font-size': '12px'}
                                ),
                        html.Div(id='attendance-chart-peer',
...
@app.callback(
    Output('user-div-peer', 'children'),
    [Input('user-div-peer', 'id')])
...
@app.callback(Output('url_logout_peer', 'pathname'),
              [Input('logout-button-peer', 'n_clicks'),
               Input('user-div-peer', 'children')])
...
@app.callback(
    [Output('attendance-chart-peer', 'children'),
     Output("legend-peer", "children"),
      Output("ES-avg-peer", "children")],
    [Input('user-div-peer', 'children')]
)
def update_graph_v1(selected_index):
    data = [
            go.Bar(
                ...
                hoverinfo="none",
                                             ...
                                             raw_data[raw_data['Student'] == selected_index][[value for value in raw_data.columns if value.startswith('ES')]].values[0]
                                             )
        ]
    ...
    ES = raw_data[[value for value in raw_data.columns if value.startswith('ES')]].select_dtypes(include=['number'])
    avg_attendance = np.round(ES.select_dtypes(include=['number']).describe().loc['mean'].mean()*100,2)

    return dcc.Graph(figure={"data":data, "layout": layout}, config={'displayModeBar': False}), legend, dcc.Markdown(f'''

             ###### You have attended {progress}% of exercise sessions so far. Average attendance of your peers is {avg_attendance}%
...
@app.callback(
    Output('ha1-output-peer', 'children'),
    [Input('user-div-peer', 'children')]
)
def update_ha1(selected_index):
    ...
    if "HA1" in raw_data.columns:
        ...
        avg_grade = np.round(np.mean(raw_data[raw_data['HA1']!= 'not submitted']['HA1']),2)
        if selected_value != 'not submitted':
            return dcc.Markdown(f'''

            ##### Your HA1 grade is {selected_value}/{max_grade}. \n 
            ###### Average grade of your peers who submitted HA1 is {avg_grade}/{max_grade}.
            ''', style = {'textAlign': 'center'})
        else:
            return dcc.Markdown(f'''
...
@app.callback(
    Output('ha2-output-peer', 'children'),
    [Input('user-div-peer', 'children')]
)
def update_ha2(selected_index):
    ...
    if "HA2" in raw_data.columns:
        ...
        avg_grade = np.round(np.mean(raw_data[(raw_data['HA2'] != 'not submitted') 
                                              & (raw_data['HA2'] != 'part 1 only')]['HA2']),2)

        if selected_value == 'part 1 only':
            return dcc.Markdown(f'''

            ##### You submitted part 1 only.
            ###### Average grade of your peers who submitted HA2 is {avg_grade}/{max_grade}.
            ''', style = {'textAlign': 'center'})
...
@app.callback(
    [Output('avgquiz-plot-peer', 'figure'),
     Output('quizzes-plot-peer', 'figure'),
     Output('models-line-plot-peer', 'figure'),
     Output('perf-avgquiz-plot-peer', 'figure'),
     Output('perf-quizzes-plot-peer', 'figure'),
    ],
    [Input('user-div-peer', 'children')]
)
def update_line_plots_v1(selected_index):
    ...
    avg_score_figure = {
        'data': [
            {'x': [value.replace(f'{i}MasteryAttemptsAvgAt', '') for value in selected_data.columns if value.startswith(f'{i}MasteryAttemptsAvgAt')],
                        'y': selected_data[[value for value in selected_data.columns if value.startswith(f'{i}MasteryAttemptsAvgAt')]].iloc[0],
                            'type': 'bar', 
                            'name': f'Your {i} score',
                            'marker': {'color': j}
                            }
    ...
    average_trend_quiz_avg = []
    exam_trend_quiz_avg = []
    anti_exam_trend_quiz_avg = []
    for i in [value.replace('MasteryAttemptsAvgAt', '') for value in selected_data.columns if value.startswith('MasteryAttemptsAvgAt')]:
        weekly_subset = raw_data[~((raw_data[f'Models{i}'] == 0 )&(raw_data[f'MasteryAttemptsAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAt{i}'] == 0 )&(raw_data[f'MasteryAttemptsAvgAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAvgAt{i}'] == 0))]
        exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 1]
        anti_exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 0]
        average_trend_quiz_avg.append(weekly_subset[[value for value in weekly_subset.columns if value.startswith(f'MasteryAttemptsAvgAt{i}')]].mean(axis=0).values[0])
        exam_trend_quiz_avg.append(exam_subset[[value for value in exam_subset.columns if value.startswith(f'MasteryAttemptsAvgAt{i}')]].mean(axis=0).values[0])
        anti_exam_trend_quiz_avg.append(anti_exam_subset[[value for value in anti_exam_subset.columns if value.startswith(f'MasteryAttemptsAvgAt{i}')]].mean(axis=0).values[0])
    ...
    average_trend_quiz_avg_perf = []
    exam_trend_quiz_avg_perf = []
    anti_exam_trend_quiz_avg_perf = []
    for i in [value.replace('PerformanceAttemptsAvgAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAvgAt')]:
        weekly_subset = raw_data[~((raw_data[f'Models{i}'] == 0 )&(raw_data[f'MasteryAttemptsAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAt{i}'] == 0 )&(raw_data[f'MasteryAttemptsAvgAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAvgAt{i}'] == 0))]
        exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 1]
        anti_exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 0]
        average_trend_quiz_avg_perf.append(weekly_subset[[value for value in weekly_subset.columns if value.startswith(f'PerformanceAttemptsAvgAt{i}')]].mean(axis=0).values[0])
        exam_trend_quiz_avg_perf.append(exam_subset[[value for value in exam_subset.columns if value.startswith(f'PerformanceAttemptsAvgAt{i}')]].mean(axis=0).values[0])
        anti_exam_trend_quiz_avg_perf.append(anti_exam_subset[[value for value in anti_exam_subset.columns if value.startswith(f'PerformanceAttemptsAvgAt{i}')]].mean(axis=0).values[0])
    ...
    for x_value, y_value in zip([value.replace('MasteryAttemptsAt', '') for value in selected_data.columns if value.startswith('MasteryAttemptsAt')],
        ...
        if x_value in missed_dict:
            ...
            annotations.append(dict(x=x_value,y=y_value+3, text=annotation_text_final, showarrow=False, font=dict(size=7), textangle = 20))
    ...
    average_trend_quiz_num = []
    exam_trend_quiz_num = []
    anti_exam_trend_quiz_num = []
    for i in [value.replace('MasteryAttemptsAt', '') for value in selected_data.columns if value.startswith('MasteryAttemptsAt')]:
        weekly_subset = raw_data[~((raw_data[f'Models{i}'] == 0 )&(raw_data[f'MasteryAttemptsAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAt{i}'] == 0 )&(raw_data[f'MasteryAttemptsAvgAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAvgAt{i}'] == 0))]
        exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 1]
        anti_exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 0]
        average_trend_quiz_num.append(weekly_subset[[value for value in weekly_subset.columns if value.startswith(f'MasteryAttemptsAt{i}')]].mean(axis=0).values[0])
        exam_trend_quiz_num.append(exam_subset[[value for value in exam_subset.columns if value.startswith(f'MasteryAttemptsAt{i}')]].mean(axis=0).values[0])
        anti_exam_trend_quiz_num.append(anti_exam_subset[[value for value in anti_exam_subset.columns if value.startswith(f'MasteryAttemptsAt{i}')]].mean(axis=0).values[0])
    ...
    average_trend_quiz_num_perf = []
    exam_trend_quiz_num_perf = []
    anti_exam_trend_quiz_num_perf = []
    for i in [value.replace('PerformanceAttemptsAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAt')]:
        weekly_subset = raw_data[~((raw_data[f'Models{i}'] == 0 )&(raw_data[f'MasteryAttemptsAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAt{i}'] == 0 )&(raw_data[f'MasteryAttemptsAvgAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAvgAt{i}'] == 0))]
        exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 1]
        anti_exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 0]
        average_trend_quiz_num_perf.append(weekly_subset[[value for value in weekly_subset.columns if value.startswith(f'PerformanceAttemptsAt{i}')]].mean(axis=0).values[0])
        exam_trend_quiz_num_perf.append(exam_subset[[value for value in exam_subset.columns if value.startswith(f'PerformanceAttemptsAt{i}')]].mean(axis=0).values[0])
        anti_exam_trend_quiz_num_perf.append(anti_exam_subset[[value for value in anti_exam_subset.columns if value.startswith(f'PerformanceAttemptsAt{i}')]].mean(axis=0).values[0])
    ...
    average_trend_models = []
    exam_trend_models = []
    anti_exam_trend_models = []
    for i in [value.replace('Models', '') for value in selected_data.columns if value.startswith('Models')]:
        weekly_subset = raw_data[~((raw_data[f'Models{i}'] == 0 )&(raw_data[f'MasteryAttemptsAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAt{i}'] == 0 )&(raw_data[f'MasteryAttemptsAvgAt{i}'] == 0 )
                                   &(raw_data[f'PerformanceAttemptsAvgAt{i}'] == 0))]
        exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 1]
        anti_exam_subset = weekly_subset[weekly_subset['PassIndicator'] == 0]
        average_trend_models.append(weekly_subset[[value for value in weekly_subset.columns if value.startswith(f'Models{i}')]].mean(axis=0).values[0])
        exam_trend_models.append(exam_subset[[value for value in exam_subset.columns if value.startswith(f'Models{i}')]].mean(axis=0).values[0])
        anti_exam_trend_models.append(anti_exam_subset[[value for value in anti_exam_subset.columns if value.startswith(f'Models{i}')]].mean(axis=0).values[0])


    models_figure['data'].append({
        'x': [value.replace('Models', '') for value in selected_data.columns if value.startswith('Models')],
        'y': average_trend_models,
        'type': 'line',
        'name': 'Average Trend',
        'line': {'dash': 'dash'},
        'marker': {'color': '#3A403D'}
    ...
    models_figure['data'].append({
        ...
        'y': exam_trend_models,
        'type': 'line',
        'name': 'Average Trend: passed Exam',
        'line': {'dash': 'dash'},
        'marker': {'color': '#4BFC28'}
    })
    ...
    models_figure['data'].append({
        'x': [value.replace('Models', '') for value in selected_data.columns if value.startswith('Models')],
        'y': anti_exam_trend_models,
        'type': 'line',
        'name': 'Average Trend: failed Exam',
        'line': {'dash': 'dash'},
        'marker': {'color': 'red'}
    })
    ...
    perf_avg_score_figure['data'].append({
        'x': [value.replace('PerformanceAttemptsAvgAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAvgAt')],
        'y': average_trend_quiz_avg_perf,
        'type': 'line',
        'name': 'Average Trend',
        'line': {'dash': 'dash'},
        'marker': {'color': '#3A403D'}  
    ...
    perf_avg_score_figure['data'].append({
        'x': [value.replace('PerformanceAttemptsAvgAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAvgAt')],
        'y': anti_exam_trend_quiz_avg_perf,
        'type': 'lines+markers',
        'name': 'Average Trend: failed Exam',
        'line': {'dash': 'dash'},
        'marker': {'color': 'red'}  
    ...
    perf_quizzes_figure['data'].append({
        'x': [value.replace('PerformanceAttemptsAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAt')],
        'y': average_trend_quiz_num_perf,
        'type': 'line',
        'name': 'Average Trend',
        'line': {'dash': 'dash'},
        'marker': {'color': '#3A403D'} 
    ...
    perf_quizzes_figure['data'].append({
        'x': [value.replace('PerformanceAttemptsAt', '') for value in selected_data.columns if value.startswith('PerformanceAttemptsAt')],
        'y': anti_exam_trend_quiz_num_perf,
        'type': 'line',
        'name': 'Average Trend: failed Exam',
        'line': {'dash': 'dash'},
        'marker': {'color': 'red'} 
...
@app.callback(
    Output('practice-test-markdown-peer', 'children'),
    [Input('user-div-peer', 'children')]
)
def update_practice_test(selected_index):
    ...
    if "Cijfer" in raw_data.columns:
        ...
        avg_grade = np.round(np.mean(raw_data[raw_data['Cijfer']!= 'not submitted']['Cijfer']),2)
        ...
        if selected_value != 'not submitted':
            ...
            ###### Average grade of your peers is {avg_grade}/{max_grade}.
